...
class Scoreboard:
    ...
    def prep_ships(self):
        ...
        for ship_number in range(self.stats.ships_left):
            self.display_ship_image(ship_number)
    
    def display_ship_image(self, number):
        """Create a ship image."""
        ship = Ship(self.ai_game)

        # Scale the ship sprite before adding it to the group
        ship.image = pygame.transform.scale(ship.image, (int(ship.rect.width * ship.scale_factor)
                                                             , int(ship.rect.height * ship.scale_factor)))
